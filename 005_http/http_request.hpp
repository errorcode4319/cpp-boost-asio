...
public:
    ...
    auto GetHost() -> const std::string& const { return m_host; }
    auto GetPort() -> uint16_t const { return m_port; }
    auto GetUri() -> const std::string& const { return m_uri; }
    auto GetId() -> uint32_t  const { return m_id; }
